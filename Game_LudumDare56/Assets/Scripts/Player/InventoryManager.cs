...
public class InventoryManager : MonoBehaviour
{
    ...
    // Vend un objet spécifique
    public void SellItem(IGameItem item)
    {
        if (Inventory.Contains(item))
        {
            ...
            {
                playerMoney += sellableItem.SellPrice;
    ...
    public List<ISellable> GetSellableItems()
    {
        List<ISellable> sellableItems = new List<ISellable>();
        foreach (var item in Inventory)
        {
            if (item is ISellable)
            {
                sellableItems.Add(item as ISellable);
            }
        }
